...
export class leveltwo extends Scene
{
    ...
    update(time: number, delta: number): void {
        ...
        // === Falling Tile Logic Start ===
        this.emptyPlatforms.forEach((tile: any) => {
            const bounds = tile.getBounds();
            const p1OnTile = Phaser.Geom.Rectangle.Overlaps(bounds, this.player1.player.getBounds());
            const p2OnTile = Phaser.Geom.Rectangle.Overlaps(bounds, this.player2.player.getBounds());

            const neitherOutlined = !this.p1hasOutline && !this.p2hasOutline;

            if (neitherOutlined && (p1OnTile || p2OnTile)) {
                if (!tile.fading) {
                    tile.fading = true;
                    tile.fadeTimer = this.time.now + 2000;
